...
class DioService extends NetworkApi {
  ...
  Future<NetworkResponse> patch(
    ...
    try {
      final response = await dioClient.patch(url, data: json.encode(payload));
      result = _handleResponse(response);
    } on DioError catch (e) {
      result = _handleError(e, result);
    } finally {
  ...
  Future<NetworkResponse> postFormData(
    ...
    try {
      List<http.MultipartFile> multiParts = await Future.wait(
        paths.map(
          (path) async => await http.MultipartFile.fromPath('files[]', path,
  ...
  Future<NetworkResponse> put(
    ...
    try {
      final response = await dioClient.put(url, data: json.encode(payload));
      result = _handleResponse(response);
    } on DioError catch (e) {
      result = _handleError(e, result);
    } finally {
